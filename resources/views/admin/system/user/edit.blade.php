...
@section('body')
    <section class="common_content">
        <div class="portlet-body" style="padding:25px 0 0 15px">
            <!-- BEGIN FORM-->
            <form action="{{route('user/edit')}}" method="post" id="form_horizontal" class="form-horizontal" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-8">
                            <div class="input-icon right">
                                <i class="fa"></i>
                                <input type="text" id="nickname" name="nickname" value="{{ $info->nickname }}" data-required="1" class="form-control" />
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-radio-list" data-error-container="#role">
                                <select id="role_id" name="role_id[]" class="mt-multiselect btn btn-default" multiple="multiple" data-label="left" data-action-onchange="true">
                                    @foreach($role_list as $role)
                                        <option value="{{$role->id}}" {{ $role->selected }}>{{$role->display_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div id="role"></div>

...
use App\Models\Diary\SkillTask;
use Carbon\Carbon;
...
class SkillTaskController extends AdminController
{
    protected $skilltaskModel;
    public function __construct(Request $request,SkillTask $skilltask)
    {
        parent::__construct($request);
        $this->skilltaskModel = $skilltask;
    }

    public function index(){
        return view('admin.diary.task.skill_task.index');
    }
    public function get_list(Request $request){
        $sort_order = intval($request->get('search_order'));
        $data = $this->skilltaskModel->getList($sort_order);
    ...
    public function change_status(Request $request){
        ...
        $res = $this->skilltaskModel->where('id',$id)->update(['is_finish'=>$status,'start_time'=>$now]);
    ...
    public function edit(Request $request){
        ...
        if($request->isMethod('post')){
            $params = $request->all();
            $status = $this->skilltaskModel->updateData($params);
            if($status){
            ...
            $info = $this->skilltaskModel->getInfoById($id);
            ...
            $data['info'] = $info;
            return view('admin.diary.task.skill_task.edit',$data);
        }
    }
    public function show(Request $request){
        $id = Hashids::decode(trim($request->get('id')))[0];

        $info = SkillTask::find($id)->toArray();

        return view('admin.diary.task.skill_task.detail',['info'=>$info]);
    }
    public function del(Request $request){
        $id = intval(Hashids::decode($request->get('id'))[0]);
        $res = $this->skilltaskModel->delItem($id);
